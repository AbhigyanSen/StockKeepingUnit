...
# ----------------- QUERY TRANSACTIONS AND SAVE (ENRICHED) -----------------
print(f"\nQuerying {len(transaction)} transaction rows...")
...
for _, t_row in tqdm(transaction.iterrows(), total=len(transaction), desc="Transaction Queries"):
    query_text = " ".join(str(val) for val in t_row if pd.notna(val))
    query_emb = get_embedding(query_text)
    ...
    if query_emb is not None:
        ...
        for rank, (idx, dist) in enumerate(zip(indices[0], distances[0])):
            ...
            # Retrieve master row
            master_row = master[master["itemcode"].astype(str) == itemcode]
            if not master_row.empty:
                master_row = master_row.iloc[0].to_dict()
            else:
                master_row = {}

            # Combine transaction + master + match info
            combined = {}
            # Transaction columns
            for col in transaction.columns:
                combined[f"TR_{col}"] = t_row[col]
            # Master columns
            for col in master.columns:
                combined[f"M_{col}"] = master_row.get(col, None)
            # Match info
            combined["rank"] = rank + 1
        ...
        for col in transaction.columns:
            combined[f"TR_{col}"] = t_row[col]
        for col in master.columns:
            combined[f"M_{col}"] = None
        combined["rank"] = None
        combined["distance"] = None
        results_list.append(combined)

# Save results to new CSV
results_df = pd.DataFrame(results_list)
results_df.to_csv("./temp/t_TR_matches.csv", index=False)
print(f"Saved enriched transaction matches to ./temp/t_TR_matches.csv")